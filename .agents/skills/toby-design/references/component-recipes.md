...
| Gateway funds banner | `UI/Primitives/GatewayFundsBanner.swift` | Durable top-of-window notice when Vercel AI Gateway or OpenRouter is out of funds. Names the gateway and the failed attempt, offers Open Settings for that provider, and stays until Close. No timer. Reduce Motion fades it. |
...
| Sidebar | `Features/Sidebar/{AppSidebar,SidebarFooter,SidebarConnectionStatus}.swift` | Native `List(selection:)` destinations (primary + Automation + Tools), system glass (no `AppTheme.sidebarBackground` fill), compact persona footer with a persistent status-dot control, and labelled connection recovery only when unhealthy. Record browsing lives in feature workspaces, not the global sidebar. When Sparkle finds a newer app version, `UpdateToolbarButton` appears in the window toolbar with a TipKit popover (`UpdateAvailableTipCard.swift`; returns after a newer version or more than two days if dismissed). |
| Input dock | `UI/Primitives/InputDock.swift` | Return sends, Shift-Return adds newline, 2–6 lines, support permitted attachments (images show a small removable thumbnail; other files stay filename chips). Discarding an attachment fades it out and collapses its space (~0.5s ease-in-out); Reduce Motion snaps. Empty-field placeholder restates the Return / Shift-Return shortcuts. Attach is a native circular button on the left of the control row; context gauge, Cancel while loading, and a larger circular Send sit on the right. Send uses `AppTheme.accent` when enabled. Enabled attach/cancel/send show a pointing-hand cursor and a quiet primary-text hover wash (16% on Send, 10% on accessory; snaps under Reduce Motion). Gauge/unavailable context state. Floating concentric `.glassEffect`. Send/cancel/attach are native circular/bordered buttons, not painted circles or nested glass. |
...
| Inspect detail rows | `UI/Primitives/DetailInspect.swift` | `DetailHeading` (title2 semibold), `DetailSection` (13pt semibold title + content), `DetailMetadataStack` + `DetailMetadataRow` (Grid: labels share a leading column, values sit immediately beside them — do not Spacer-split label and value to opposite edges). 20–22pt section spacing on `SettingsDesign.canvasBackground`. No SettingsCard chrome on the primary inspect pane. |
...
| Onboarding/flow runner | `Features/Dashboard/{OnboardingCard,DashboardActionRunnersRail}.swift` | Make setup/action outcomes explicit; Actions runners are Shortcuts-style colored tiles (flow icon, play glyph, and name; not title rows); preserve dashboard editing/reordering behavior rather than rebuilding cards. |
