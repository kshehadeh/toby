...
| Sidebar | `Features/Sidebar/{AppSidebar,SidebarFooter,SidebarConnectionStatus,UpdateAvailableTipCard}.swift` | Native `List(selection:)` destinations (primary + Automation + Tools), system glass (no `AppTheme.sidebarBackground` fill), compact persona footer with a persistent status-dot control, labelled connection recovery only when unhealthy, and an inline TipKit update card under the destination list when Sparkle finds a newer app version (returns after a newer version or more than two days if dismissed). Record browsing lives in feature workspaces, not the global sidebar. |
