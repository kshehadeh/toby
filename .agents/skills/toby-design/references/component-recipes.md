...
| Onboarding/flow runner | `Features/Dashboard/{OnboardingCard,DashboardActionRunnersRail}.swift` | Make setup/action outcomes explicit; Actions runners are 64×64 flow icons (not title rows); preserve dashboard editing/reordering behavior rather than rebuilding cards. |
