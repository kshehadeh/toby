...
| Input dock | `UI/Primitives/InputDock.swift` | Return sends, Shift-Return adds newline, 2–6 lines, support permitted attachments (images show a small removable thumbnail; other files stay filename chips). Discarding an attachment fades it out and collapses its space (~0.5s ease-in-out); Reduce Motion snaps. Gauge/unavailable context state, Cancel while loading. Floating concentric `.glassEffect`. Send/cancel/attach are native circular/borderless buttons, not painted circles or nested glass. |
