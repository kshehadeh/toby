...
| Recordings | `Features/Recordings/{RecordingsView,RecordingsDetailView,RecordingDetailContent}.swift` | Account for active capture, processing, error, empty, single detail, and multi-selection. Do not auto-select a saved or in-progress recording. Select a newly started recording; keep the last selection when leaving and returning. |
