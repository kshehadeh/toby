...
| Browse and inspect | Same `FeatureWorkspaceSplit` as Chats: a second list column (`FeatureBrowserList` + `FeatureBrowserRow`, 10pt horizontal inset) plus selected detail, both under the window toolbar separator. Unselected detail is `FeatureBrowserPlaceholder` (type icon + select/create copy). Do not use card-grid overviews. Preserve trailing inspectors (project files/chats). |
