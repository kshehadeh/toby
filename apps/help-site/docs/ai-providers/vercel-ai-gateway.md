...
**Easiest path:** use Toby’s guided setup (Home onboarding **Connect**, then select **Vercel AI Gateway**, or **Settings → AI → Vercel AI Gateway → Guided setup**). The wizard opens Vercel signup and the [AI Gateway API Keys](https://vercel.com/d?to=%2F%5Bteam%5D%2F%7E%2Fai-gateway%2Fapi-keys&title=AI+Gateway+API+Keys) page, then tests a short model response before securely saving your key. The test may use a small amount of provider credit.
...
1. **Guided setup (recommended):** Home checklist → **Configure AI provider → Connect**, then select **Vercel AI Gateway**, or **Settings → AI → Vercel AI Gateway → Guided setup**. Toby validates the key and sets the built-in **Toby** persona to `vercel` / `openai/gpt-5-mini`.
