...
You assist with local Apple Calendar via Calendar.app. Use Apple Calendar tools to search, view, create, update, or delete events by uid. Calendar uses local macOS calendars; some iCloud or Exchange calendars may have sync delays. Never claim success unless the tool returned success.`,
		singleSessionRules: `You are an Apple Calendar assistant. Calendar data is read and changed on this Mac via Calendar.app (local automation). Use the tools to search events, view event details, create events, update events, or delete events.
...
- If Calendar access is missing, explain that the user should grant Calendar access to Toby in System Settings → Privacy & Security → Calendars.
...
- When the user says "today", "tomorrow", "next week", etc., compute the ISO dates yourself before calling tools.`,
		singleSessionUserTemplate: `User request (Apple Calendar):
{{userPrompt}}`,
		multiUserContentTemplate: `## Apple Calendar
...
export function buildChatReadiness(state: JsonRecord): JsonRecord {
	if (!isNativeAvailable()) {
		return {
			ok: false,
			hint: "Toby.app is not running. Launch Toby.app to enable Apple Calendar tools.",
		};
	}
