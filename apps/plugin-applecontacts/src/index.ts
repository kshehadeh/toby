#!/usr/bin/env bun
/**
 * Apple Contacts installable Toby plugin (protocol v1, bun-package).
 *
 * This plugin delegates contact reads to Toby.app's native API server
 * (localhost). The TypeScript plugin is a thin protocol adapter that forwards
 * tool executions to the app's Contacts.framework-backed handler.
 */
...
const DISPLAY_NAME = "Apple Contacts";
const DESCRIPTION =
	"Read local Contacts.app information on macOS — search contacts and view details";

const RESOURCES = ["contacts"];

function validateSubtools(): JsonRecord[] {
	const checks: JsonRecord[] = [];
	const r = nativeRequest("contacts/search", { limit: 1 });
	checks.push({
		tool: "searchContacts",
		ok: r.ok,
		details: r.ok ? "search completed" : (r.error ?? "failed"),
	});
	checks.push({
		tool: "getContact",
		ok: true,
		details: "Not executed; requires a contact identifier from searchContacts.",
	});
	return checks;
}
...
function handleStatus(
	...
	if (!nativeAvailable) {
		...
		payload.details =
			"Toby.app is not running. Launch Toby.app to enable Apple Contacts tools.";
		emitJson(payload);
	}

	if (connected) {
		const smoke = nativeRequest("contacts/search", { limit: 1 });
		if (smoke.ok) {
			payload.details = "Contacts.app reachable.";
		} else {
			payload.ok = false;
			payload.details = `Connected, but Contacts.app check failed: ${smoke.error ?? "unknown"}`;
		}
	} else {
		payload.details =
			"Apple Contacts is not connected. Run `toby connect applecontacts` on this Mac first.";
	}
	...
	if (validateTools && connected) {
		...
		if (failed.length === 0) {
			payload.details = `Contacts.app reachable; validated ${toolChecks.length} tool check(s).`;
			payload.ok = true;
		} else {
			payload.ok = false;
		...
		payload.details =
			"Apple Contacts is configured; full Contacts.app validation skipped.";
	}
...
function handleConnect(): never {
	const access = nativeRequest("contacts/request-access");
	if (!access.ok) {
		emitJson({
			ok: false,
			reason: `Contacts.app access request failed: ${access.error ?? "unknown"}`,
		});
	}
	const smoke = nativeRequest("contacts/search", { limit: 1 });
	if (smoke.ok) {
		emitJson({ ok: true, reason: "Apple Contacts connected successfully." });
	}
	emitJson({
		ok: false,
		reason: `Contacts.app check failed: ${smoke.error ?? "unknown"}`,
	});
}

function handleDisconnect(): never {
	emitJson({ ok: true, reason: "Apple Contacts disconnected." });
}
