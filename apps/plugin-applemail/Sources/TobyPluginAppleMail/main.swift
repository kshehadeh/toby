import Foundation
import TobyPluginAppleMailLib

@main
enum TobyPluginAppleMail {
	static func main() {
	...
	private static func handleStatus(envelope: ConfigEnvelope) {
		...
		if !MailClient.isPlatformSupported {
			...
			payload["details"] = "Apple Mail is only available on macOS."
			PluginOutput.emit(payload)
		}

		if connected {
			do {
				try MailClient.testConnection()
				payload["details"] = "Mail.app reachable."
			} catch {
				payload["ok"] = false
				payload["details"] = "Connected, but Mail.app check failed: \(error.localizedDescription)"
			}
		} else {
			payload["details"] =
		...
		if envelope.validateTools && connected {
			let toolChecks = MailClient.validateTools()
			payload["tools"] = toolChecks
			let failed = toolChecks.filter { ($0["ok"] as? Bool) == false }
			if failed.isEmpty {
				payload["details"] = "Mail.app reachable; validated \(toolChecks.count) tool check(s)."
				payload["ok"] = true
			} else {
				payload["ok"] = false
				payload["details"] =
			...
			payload["details"] = "Apple Mail is configured; full Mail.app validation skipped."
		}
	...
	private static func handleConnect(envelope: ConfigEnvelope) {
		guard MailClient.isPlatformSupported else {
			PluginOutput.emit(["ok": false, "reason": "Apple Mail integration is only available on macOS."])
		}
		do {
			try MailClient.testConnection()
			PluginOutput.emit(["ok": true, "reason": "Apple Mail connected successfully."])
		} catch {
			PluginOutput.emit(["ok": false, "reason": "Could not reach Mail.app: \(error.localizedDescription)"])
		}
	}

	private static func handleDisconnect() {
		PluginOutput.emit(["ok": true, "reason": "Apple Mail disconnected."])
	}
	...
	private static func handleToolsList() {
		PluginOutput.emit(["ok": true, "tools": MailTools.definitions])
	}
	...
	private static func handleToolsExecute(raw: String) {
		...
		guard MailTools.definitions.contains(where: { ($0["name"] as? String) == tool }) else {
			PluginOutput.emit(["ok": false, "error": "Unknown tool: \(tool)"])
		}

		switch MailTools.execute(tool: tool, input: input, dryRun: dryRun) {
