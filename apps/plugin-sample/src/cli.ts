#!/usr/bin/env bun
/**
 * Reference installable Toby plugin (protocol v1).
 * Build: bun run build (from this directory) or `bun run build:plugin:sample` from repo root.
 */
...
function handleDisconnect(): never {
	emitJson({ ok: true, reason: "Sample plugin disconnected." });
}
		...
		fields: [
			{
				...
				required: true,
				masked: true,
				description: "Demo credential used by the sample plugin",
			},
			{
				key: "greeting",
...
function handleConfigSet(): never {
	emitJson({ ok: true, reason: "Sample plugin config synced." });
}
