...
async function resolveSlackAuthToken(
	config: JsonRecord,
	...
	const creds = getSlackCredentials(config);
	if (
		creds.authMethod === "oauth" &&
		!isSlackAccessTokenFresh(creds.oauthExpiresAt)
	) {
		const refreshed = await refreshSlackOAuthAccessToken(config);
...
export function resolveSlackPostToken(config: JsonRecord): string {
	const manualBot = getConfigField(config, "botToken");
	const oauthBot = getConfigField(config, "oauthBotToken");
	if (manualBot || oauthBot) {
		return manualBot || oauthBot || "";
	}
	return getSlackCredentials(config).botToken;
}
...
async function slackApiResult<T extends SlackApiResponse>(
	...
	let authToken = await resolveSlackAuthToken(config, token);
	...
	if (
		...
		getSlackAuthMethod(config) === "oauth"
	) {
		const refreshed = await refreshSlackOAuthAccessToken(config);
...
export async function resolveSlackBotUserId(
	...
	const auth = await testSlackConnection(config, botToken);
	...
	throw new Error(
		"Could not resolve Slack bot user id from auth.test. Set botUserId in configure.",
	);
}

export function getSlackInboundCredentials(config: JsonRecord): {
	...
	if (!botToken) {
		const userOAuth = getConfigField(config, "oauthUserToken");
		if (userOAuth) {
			throw new Error(
				"Slack inbound needs a bot token (xoxb-...). `toby connect slack` only stores a user token (xoxp-...) for chat tools. In `toby configure` → Slack, paste Bot Token (OAuth & Permissions → install app → Bot User OAuth Token) and App Token (xapp-..., Socket Mode).",
			);
		}
		throw new Error(
			"Slack inbound requires a bot token (xoxb-...) and an app token (xapp-...) for Socket Mode. Add both in `toby configure` → Slack.",
		);
	}
	const appToken = getConfigField(config, "appToken");
	if (!appToken) {
		throw new Error(
	...
	const botUserId = getConfigField(config, "botUserId");
	return {
		botToken,
		appToken,
		botUserId,
...
export async function searchSlackUsers(
	...
	if (q && looksLikeEmail(q)) {
		...
		try {
			const byEmail = await lookupUserByEmail(config, q);
	...
	if (q) {
		const roster = await listUsersPaginated(config, 1000);
		...
		const sample = await listUsers(config, Math.min(limit, 200));
...
export async function searchConversations(
	...
	const [channels, users] = await Promise.all([
		listConversations(config, 500),
		listUsers(config, 500),
	]);
...
export async function resolveChannelId(
	...
	const conversations = await listConversations(config, 500);
	const byName = conversations.find((c) => c.name.toLowerCase() === normalized);
	if (byName) return byName.id;

	const users = await listUsers(config, 500);
	const byUser =
	...
	if (byUser) {
		const opened = await slackApi<{
			...
			error?: string;
			channel?: { id?: string };
		}>(config, "conversations.open", { users: byUser.id });
