...
| Path | Role |
| --- | --- |
| [`src/ai/caching/index.ts`](../src/ai/caching/index.ts) | Orchestrator entry: `applyChatPromptCaching`, public exports |
| [`src/ai/caching/types.ts`](../src/ai/caching/types.ts) | `CacheAdapter`, `TokenUsageReport`, `ChatCacheContext` |
| [`src/ai/caching/registry.ts`](../src/ai/caching/registry.ts) | Maps `persona.ai.provider` → adapter |
| [`src/ai/caching/shared.ts`](../src/ai/caching/shared.ts) | Stable `promptCacheKey`, `mergeProviderOptions`, gateway slug parsing |
| [`src/ai/caching/usage.ts`](../src/ai/caching/usage.ts) | `extractTokenUsageReport`, status-line and debug formatting |
| [`src/ai/caching/adapters/openai.ts`](../src/ai/caching/adapters/openai.ts) | Direct OpenAI |
| [`src/ai/caching/adapters/vercel-gateway.ts`](../src/ai/caching/adapters/vercel-gateway.ts) | Vercel AI Gateway |

[`src/ai/cache-hints.ts`](../src/ai/cache-hints.ts) re-exports the orchestrator for older imports; new code should import from `src/ai/caching`.
...
Each adapter implements [`CacheAdapter`](../src/ai/caching/types.ts):
...
Register new providers in [`src/ai/caching/registry.ts`](../src/ai/caching/registry.ts).
...
Built in [`buildStablePromptCacheKey`](../src/ai/caching/shared.ts). The key is:
...
If you change the stable system prompt layout in a breaking way, bump `DEFAULT_CHAT_PROMPT_SCHEMA_VERSION` in [`src/ai/caching/shared.ts`](../src/ai/caching/shared.ts).
...
[`formatTokenUsageStatus`](../src/ai/caching/usage.ts) drives the chat input dock footer, for example:
...
Set `TOBY_DEBUG_CACHE=1` to append a meta line per turn with `cacheRead`, `cacheWrite`, and `noCache` via [`formatCacheDebugMeta`](../src/ai/caching/usage.ts) in [`src/ui/chat/chat-session-app.tsx`](../src/ui/chat/chat-session-app.tsx).

### Turn logs

[`logTurnSummary`](../src/logging/chat-log.ts) receives `cacheReadTokens` and `cacheWriteTokens` from `extractTokenUsageReport` in the chat session app.
...
1. Create `src/ai/caching/adapters/<id>.ts`:
...
2. Register in [`src/ai/caching/registry.ts`](../src/ai/caching/registry.ts).
