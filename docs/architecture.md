...
| `toby.appearance.showDashboardNews` | **true** | Show news | Show/hide the news card on the home dashboard |
| `toby.appearance.hideOnboarding` | **false** | Hide onboarding checklist | Hide the setup checklist even if steps are incomplete. Set from the Home checklist **Hide** control or Settings → Home |
