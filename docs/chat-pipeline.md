...
- **`webSearch`** — Searches the web via Perplexity through the Vercel AI Gateway (`gateway.tools.perplexitySearch()`). Returns titles, URLs, snippets, and optional dates. Available as a **conditional global tool** when web search is enabled in Settings and the persona's AI provider is Vercel AI Gateway. When available, it is always included in the tool set (protected from pretreatment filtering via `ALWAYS_INCLUDED_TOOLS`). See [`web-search.md`](web-search.md) and [`packages/core/src/ai/web-search-global-tools.ts`](../packages/core/src/ai/web-search-global-tools.ts).
