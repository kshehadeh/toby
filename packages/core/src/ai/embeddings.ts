...
/** Resolve the embedding model id for the persona's AI provider. */
export function resolveEmbedModelId(persona: Persona): string {
...
/** @deprecated Use {@link resolveEmbedModelId}. Kept for routing call sites. */
export const resolveRoutingEmbedModelId = resolveEmbedModelId;

export function createEmbeddingModelForPersona(
	persona: Persona,
): EmbeddingModel | null {
	const modelId = resolveEmbedModelId(persona);
	try {
