...
/**
 * Rank entities by cosine similarity to a query vector; return up to topK
 * scores meeting minScore (in descending score order).
 */
export function searchTopKScoredByCosine(params: {
...
export function searchTopKByCosine(params: {
	...
	return searchTopKScoredByCosine(params).map((s) => s.id);
}
...
export function bufferToVector(blob: Buffer | Uint8Array): number[] {
