...
function lifecycleStatusEmoji(headerOrLine: string): string {
	...
	if (lower.includes("chatting with") || lower.includes("model")) return "🤖";
	if (lower.includes("save") || lower.includes("persist")) return "💾";
	if (lower.includes("integration") || lower.includes("context")) return "🔌";
	if (lower.includes("boot") || lower.includes("start")) return "🚀";
	if (lower.includes("expand") || lower.includes("prep")) return "⏳";
	return "⚙️";
}

function slackStatusEmojiForChatEvent(event: ChatEvent): string {
	switch (event.type) {
		case "prep_start":
			return "⏳";
		case "prep_end":
...
function emojiForProgressLine(line: string, event: ChatEvent): string {
	...
	if (line.startsWith("Listening to what ")) return "🧠";
	if (line.startsWith("Chatting with ")) return "🤖";
	if (line.startsWith("Executing phase")) return "🚀";
...
/** Slack status line: emoji + italic mrkdwn for inbound progress UI. */
