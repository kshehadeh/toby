...
Launch the app against it:
  TOBY_DIR="$HOME/.toby-generic" bun run app
  # or for a running binary / Dev app that inherits env:
  TOBY_DIR="$HOME/.toby-generic" open -n "dist/Toby (Dev).app" --env TOBY_DIR="$HOME/.toby-generic"
"""
...
# optional: keep credentials as plaintext placeholders
export TOBY_CREDENTIALS_KEY_BACKEND=plaintext

# CLI against this home
bun run --filter @toby/cli start -- status

# Native app (Dev) — pass TOBY_DIR into the process environment
...
def main() -> None:
    ...
    print()
    print("Launch with:")
    print(f'  TOBY_DIR="{dest}" TOBY_CREDENTIALS_KEY_BACKEND=plaintext bun run app')
